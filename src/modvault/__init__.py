...
class ModItemDelegate(QtWidgets.QStyledItemDelegate):
    ...
    def paint(self, painter, option, index, *args, **kwargs):
        ...
        clip = QtCore.QRectF(
            0, 0,
            option.rect.width() - iconsize.width() - 15, option.rect.height(),
        )
    ...
    def sizeHint(self, option, index, *args, **kwargs):
        ...
        return QtCore.QSize(
            ModItem.ICONSIZE + ModItem.TEXTWIDTH + ModItem.PADDING,
            ModItem.ICONSIZE + ModItem.PADDING,
        )
...
class ModItem(QtWidgets.QListWidgetItem):
    ...
    def __init__(self, parent, uid, *args, **kwargs):
        ...
        self._mod_dl_request = DownloadRequest()
        self._mod_dl_request.done.connect(self._on_mod_downloaded)
        ...
        if self.thumbstr == "":
            ...
            if img:
                ...
                self.parent.client.mod_downloader.download_preview(
                    name[:-4], self._mod_dl_request, self.thumbstr,
                )
    ...
    def shouldBeVisible(self):
        ...
        if p.searchString != "":
            if not (
                self.author.lower().find(p.searchString) != -1
                or self.name.lower().find(p.searchString) != -1
                or (
