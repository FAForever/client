...
class Relay(QObject):
    ...
    def send(self, message):
        self._logger.info("game at 127.0.0.1:{}<<{} len: {}".format(self.game_port, self.peer_id, len(message)))
    ...
    def _ready_read(self):
        while self._socket.hasPendingDatagrams():
            ...
            self._logger.info("{}>>{}/{}".format(self._socket.localPort(), self.login, self.peer_id))
            self.recv(data)
